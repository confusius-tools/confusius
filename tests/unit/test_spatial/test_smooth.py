...
class TestSmoothVolume:
    ...
    def test_matches_scipy_3d(self, sample_voxeldata_3d):
        """smooth_volume should match scipy.ndimage.gaussian_filter on a 3D volume."""
        vol = sample_voxeldata_3d
        fwhm = 0.4
    ...
    def test_matches_scipy_4d_skips_time(self, sample_voxeldata_3dt):
        """Time dimension should not be smoothed (sigma=0)."""
        vol = sample_voxeldata_3dt
        fwhm = 0.4
    ...
    def test_anisotropic_fwhm_dict(self, sample_voxeldata_3d):
    ...
    def test_selected_dims_only(self, sample_voxeldata_3d):
    ...
    def test_fwhm_dict_infers_smoothed_dims(self, sample_voxeldata_3d):
    ...
    def test_fwhm_dict_can_smooth_time(self, sample_voxeldata_3dt):
    ...
    def test_preserves_coords_and_attrs(self, sample_voxeldata_3d):
    ...
    def test_zero_fwhm_is_identity(self, sample_voxeldata_3d):
    ...
    def test_nans_propagate_by_default(self, sample_voxeldata_3d):
        """NaNs propagate to neighbouring voxels when ensure_finite=False (default)."""
        vol = sample_voxeldata_3d.copy()
    ...
    def test_ensure_finite_suppresses_nan_propagation(self, sample_voxeldata_3d):
    ...
    def test_dask_chunked_time_ok(self, sample_voxeldata_3dt):
    ...
    def test_raises_invalid_dim(self, sample_voxeldata_3d):
    ...
    def test_raises_unknown_fwhm_key(self, sample_voxeldata_3d):
        """Should raise ValueError if fwhm dict contains dim names not in the array."""
        with pytest.raises(ValueError, match="not present in the DataArray"):
            smooth_volume(sample_voxeldata_3d, fwhm={"k": 0.3, "w": 0.2})

    def test_raises_chunked_spatial_dim(self, sample_voxeldata_3d):
        """Should raise ValueError if a smoothed spatial dim is Dask-chunked."""
        pytest.importorskip("dask.array")
        vol_dask = sample_voxeldata_3d.chunk({"k": 2})  # Spatial dim chunked.
