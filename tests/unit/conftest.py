...
def make_sample_voxeldata_mask(sample_voxeldata_3dt):
    ...
    def _make(
        ...
        mask = xr.DataArray(
            np.zeros(
                tuple(sample_voxeldata_3dt.sizes[dim] for dim in spatial_dims),
                dtype=dtype,
            ),
            dims=spatial_dims,
            coords={
...
# ── Atlas fixtures ──────────────────────────────────────────────────────────────────
# Shared by the atlas and plotting unit tests: both need a fully controlled atlas
# Dataset (with a real mesh file) without hitting the network.
...
def mock_mesh() -> mio.Mesh:
    ...
    The atlas midline is at 100 µm (shape[2]=8, resolution=50 µm).
    """
    points = np.array(
        [
            [0.0, 0.0, 50.0],
...
def child_obj_path(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    mesh_dir = tmp_path_factory.mktemp("child_meshes")
    path = mesh_dir / "10"
    path.write_bytes(
        DracoPy.encode(points[:, ::-1] * 1000.0, faces[:, ::-1], preserve_order=True)
    )
    return path
...
def atlas_ds(structure_list: list[dict], mock_structures: StructuresDict) -> xr.Dataset:
    ...
    hemispheres_data[:, :, :2] = 2  # Right: RL below the 0.1 mm mesh midline.
    hemispheres_data[:, :, 2:] = 1  # Left: RL at or above the 0.1 mm mesh midline.
