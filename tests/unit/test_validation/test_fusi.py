...
def test_validate_fusi_accepts_valid_3d(
    sample_3d_volume: xr.DataArray,
) -> None:
    """A canonical 3D spatial DataArray validates successfully."""
    validate_fusi(sample_3d_volume)


def test_validate_fusi_accepts_singleton_z(
    ...
    validate_fusi(single_slice)


def test_validate_fusi_accepts_valid_3dt(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """A canonical 3D+t DataArray validates successfully."""
    validate_fusi(sample_3dt_volume)


def test_validate_fusi_rejects_non_dataarray() -> None:
    """Non-DataArray inputs raise `TypeError`."""
    with pytest.raises(TypeError, match="xarray.DataArray"):
        validate_fusi(np.zeros((2, 2)))  # type: ignore


def test_canonicalize_fusi_rejects_non_dataarray() -> None:
    """Canonicalization also validates input type."""
    with pytest.raises(TypeError, match="xarray.DataArray"):
        canonicalize_fusi(np.zeros((2, 2)))  # ty: ignore[invalid-argument-type]


def test_ensure_fusi_rejects_non_dataarray() -> None:
    """Ensure validates input type before inspecting dimensions."""
    with pytest.raises(TypeError, match="xarray.DataArray"):
        ensure_fusi(np.zeros((2, 2)))  # ty: ignore[invalid-argument-type]


@pytest.mark.parametrize(("dim", "index"), [("z", 1), ("y", 2), ("x", 3)])
def test_canonicalize_fusi_restores_scalar_indexed_spatial_dim(
    sample_3dt_volume: xr.DataArray,
    dim: str,
    index: int,
) -> None:
    ...
    result = canonicalize_fusi(sliced)

    assert_identical(result, expected)


def test_canonicalize_fusi_restores_multiple_spatial_dims_with_extra_dim(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_canonicalize_fusi_rejects_missing_non_scalar_spatial_coord(
    ...
    with pytest.raises(ValueError, match="missing spatial dimension 'z'"):
        canonicalize_fusi(data)


@pytest.mark.parametrize("spatial_dim", ("z", "y", "x"))
def test_ensure_fusi_returns_canonicalized_valid_data(
    sample_3dt_volume: xr.DataArray,
    spatial_dim: str,
) -> None:
    """`ensure_fusi` restores every scalar-indexed spatial dimension."""
    sliced = sample_3dt_volume.isel({spatial_dim: 0})
    ...
    assert_identical(result, sample_3dt_volume.isel({spatial_dim: [0]}))


def test_canonicalize_fusi_restores_all_spatial_dims_from_scalars() -> None:
...
def test_validate_fusi_applies_time_series_options(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """`validate_fusi` applies optional temporal constraints."""
    irregular = sample_3dt_volume.assign_coords(
        time=[
            0.0,
            1.0,
            2.0,
            4.0,
            *range(5, sample_3dt_volume.sizes["time"] + 1),
        ]
    )
    irregular.coords["time"].attrs["units"] = "s"

    with pytest.raises(ValueError, match="Non-uniform 'time' coordinates"):
        validate_fusi(irregular, require_uniform_time=True)


def test_validate_fusi_requires_unchunked_time(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """`validate_fusi` rejects multiple time chunks when requested."""
    chunked = sample_3dt_volume.chunk(time=2)

    with pytest.raises(ValueError, match="chunked along the 'time' dimension"):
        validate_fusi(chunked, require_unchunked_time=True)


def test_ensure_fusi_applies_prevalidation_options(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """`ensure_fusi` preserves validator failures before canonicalizing."""
    with pytest.raises(ValueError, match="must not have a 'pose' dimension"):
        ensure_fusi(sample_3dt_volume.expand_dims(pose=[0]), allow_pose=False)

    with pytest.raises(ValueError, match="must have a 'time' dimension"):
        ensure_fusi(sample_3dt_volume.isel(time=0), require_time=True)


def test_ensure_fusi_require_time_requires_multiple_timepoints(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """`require_time=True` rejects a single-frame fUSI series."""
    with pytest.raises(ValueError, match="requires more than 1 timepoint"):
        ensure_fusi(sample_3dt_volume.isel(time=[0]), require_time=True)


def test_validate_fusi_allows_extra_dims_by_default(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_rejects_non_monotonic_numeric_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_allows_non_monotonic_numeric_extra_dimension_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_allows_non_dimension_coordinates(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_can_forbid_extra_dims(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_requires_full_spatial_trio(
    sample_3dt_volume: xr.DataArray,
) -> None:
    ...
    with pytest.raises(ValueError, match=r"must contain all spatial dimensions"):
        validate_fusi(dropped_z)

    dropped_zy = sample_3dt_volume.isel(z=0, y=0, drop=True)
    with pytest.raises(ValueError, match=r"must contain all spatial dimensions"):
        validate_fusi(dropped_zy)


@pytest.mark.parametrize("dims", [("y", "x"), ("time", "y", "x")])
def test_validate_fusi_rejects_2d_spatial_layouts(
    dims: tuple[str, ...],
) -> None:
...
def test_validate_fusi_can_require_time(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_rejects_missing_dimension_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_allows_missing_extra_dimension_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_rejects_non_numeric_core_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_can_require_canonical_dim_order(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_can_require_regular_spacing(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_can_target_space_dims_only(
    sample_3dt_volume: xr.DataArray,
) -> None:
    ...
    bad_time.coords["time"].attrs["units"] = "s"
...
def test_validate_fusi_regular_spacing_rejects_missing_selected_dim(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_rejects_missing_single_selected_dim(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_space_regular_spacing_excludes_pose() -> None:
    ...
    for dim in ("z", "y", "x"):
        data.coords[dim].attrs["units"] = "mm"

    validate_fusi(data, require_regular_spacing=True)


def test_validate_fusi_regular_spacing_skips_non_numeric_selected_dims(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_all_checks_all_numeric_dims(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_all_skips_non_numeric_dims(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_all_requires_extra_dim_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_requires_singleton_spatial_voxdim(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """Singleton spatial axes require explicit physical width metadata."""
    bad = sample_3dt_volume.isel(y=[0]).copy(deep=True)
...
def test_validate_fusi_requires_time_units(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """Time coordinate must define units metadata."""
...
def test_validate_fusi_rejects_non_finite_numeric_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_requires_spatial_units(
    sample_3dt_volume: xr.DataArray,
) -> None:
    """Spatial coordinates must define units metadata."""
    bad = sample_3dt_volume.copy(deep=True)
...
def test_validate_fusi_rejects_non_dimension_coordinate(
    sample_3dt_volume: xr.DataArray,
) -> None:
...
def test_validate_fusi_regular_spacing_still_requires_numeric_core_coords(
    sample_3dt_volume: xr.DataArray,
) -> None:
