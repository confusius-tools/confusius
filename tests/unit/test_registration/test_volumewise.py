...
class TestRegisterVolumewise:
    ...
    def test_invalid_intensity_scaling_raises(
        ...
        with pytest.raises(ValueError, match="Invalid moving_intensity_scaling"):
            register_volumewise(
                sample_voxeldata_2dt_registration,
                moving_intensity_scaling=intensity_scaling,
            )

    def test_fixed_intensity_scaling_without_fixed_raises(
        self, sample_voxeldata_2dt_registration
    ):
        """`fixed_intensity_scaling` without a `fixed` volume raises ValueError."""
        with pytest.raises(ValueError, match="only applies to a 'fixed' volume"):
            register_volumewise(
                sample_voxeldata_2dt_registration, fixed_intensity_scaling="db"
            )

    def test_float_intensity_scaling_is_forwarded_to_register_volume(
        self, sample_voxeldata_2dt_registration, monkeypatch
    ):
        """moving_intensity_scaling is forwarded as both fixed and moving scaling."""
    ...
    def test_fixed_intensity_scaling_is_forwarded_to_register_volume(
        self, sample_voxeldata_2dt_registration, monkeypatch
    ):
        """With `fixed`, each side gets its own scaling."""
        import confusius.registration.volumewise as volumewise_module
        ...
        register_volumewise(
            ...
            n_jobs=1,
            transform="translation",
            fixed=sample_voxeldata_2dt_registration.mean("time"),
            fixed_intensity_scaling="db",
            moving_intensity_scaling=2.0,
        )

        assert calls and all(scaling == ("db", 2.0) for scaling in calls)
